...
//        vid = new Video(title,username, views,time,videoUrl,likeCount);
//        vid.setId(id);
//        // Get the list of comments from the intent
//        commentsList = (List<String>) getIntent().getSerializableExtra("comments_list");
//
//        // Set video details
...
import com.example.youtube_android.Comment;
import com.example.youtube_android.CommentsAdapter;
import com.example.youtube_android.Video;
import com.example.youtubeandroid.R;
...
public class VideoPage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String views = getIntent().getStringExtra("video_views");
        String time = getIntent().getStringExtra("video_time");
        String time_type = getIntent().getStringExtra("video_time_type");
        String videoUrl = getIntent().getStringExtra("video_url");
        likeCount = getIntent().getIntExtra("like_count", 0);
        int id = getIntent().getIntExtra("id", 0);
        vid = new Video(title,username, views,time,videoUrl,likeCount);
        vid.setId(id);
        // Get the list of comments from the intent
        commentsList = (ArrayList<Comment>) getIntent().getSerializableExtra("comments_list");

        // Set video details
        videoTitle.setText(title);
        videoUsername.setText(username);
        videoViews.setText(views + " צפיות");
        videoTime.setText(time + " " + time_type);
